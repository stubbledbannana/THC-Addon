...
public class DiscordNotifs extends Module
{
    ...
    {
        super(THCAddon.CATEGORY, "discord-notifs", "Sends notifications to a discord webhook.");
    }
        ...
        new Thread(() -> {
            try {
                ...
                if (responseCode == 204 || responseCode == 200) {
                    THCAddon.LOG.info("Successfully sent message to webhook!");
                } else {
                    THCAddon.LOG.warn("Webhook response code: " + responseCode);
                    THCAddon.LOG.warn("Failed to send to Webhook");
                }

                conn.disconnect();
            } catch (Exception e) {
                THCAddon.LOG.warn("Failed to send to webhook: " + e.getMessage());
            }
        }).start();
    }
